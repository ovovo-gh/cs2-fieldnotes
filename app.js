(() => {
  ...
  let tacticsMapId='dust2',tacticPlaceUtility='smoke',tacticPlaceSide='T',tacticsLineupId='',tacticsPlacementMode=false,tacticThrowSerial=0,tacticLineupQuery='',tacticLineupFilter='all',tacticLineupEditingId=null;
  ...
  function updateTacticTimelineUI(){
    ...
    const mapTime=$('#tactic-map-live-time'),mapStatus=$('#tactic-map-live-status'),placeCue=$('.map-place-cue'),editorHint=$('.editor-hint');if(mapTime)mapTime.textContent=`时间轴 ${tacticDisplayTime(time)} 秒`;if(mapStatus)mapStatus.textContent=tacticTimelineMapStatus(selectedTacticMap(),time);if(placeCue)placeCue.textContent=tacticsPlacementMode?`点击地图放置 · 当前时刻 ${tacticDisplayTime(time)} 秒`:'';if(editorHint)editorHint.textContent=`当前时间 ${tacticDisplayTime(time)} 秒 · 选择“点击地图添加”后，在地图上点击实际落点；投掷时间和落点都会记录到下方时间轴。`;
  ...
  function tacticLineupFormDefaults(map){return {mapId:map.id,utility:'smoke',side:'T',name:'',from:{label:'站位',x:50,y:50},target:{label:'落点',x:50,y:50},technique:'左键',movement:'站定',airTime:'',steps:[],result:'',source:'',updated:today(),media:{poster:'',video:''}};}
  ...
  document.addEventListener('click',e=>{
    ...
    if(t.dataset.mapId){if(tacticMaps.some(map=>map.id===t.dataset.mapId)){tacticsMapId=t.dataset.mapId;tacticsLineupId='';tacticLineupEditingId=null;tacticsPlacementMode=false;render();}return;}
    ...
    if(t.id==='tactic-lineup-new'){tacticLineupEditingId='new';render({preserveScroll:true});$('#tactic-lineup-name')?.focus();return;}
    if(t.id==='tactic-lineup-cancel'||t.id==='tactic-lineup-cancel-bottom'){tacticLineupEditingId=null;render({preserveScroll:true});return;}
  ...
  document.addEventListener('submit',e=>{
    if(e.target.id==='tactic-lineup-form'){
      ...
      const utility=tacticUtilityIDs.has(String(f.get('utility')||''))?String(f.get('utility')):'smoke',side=String(f.get('side')||'')==='CT'?'CT':'T',readCoord=key=>numberIn(f.get(key),0,100,50),existing=tacticLineupEditingId&&tacticLineupEditingId!=='new'?customTacticLineupById(tacticLineupEditingId):null,id=existing?.id||`custom-${map.id}-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;
      const record={id,mapId:map.id,utility,side,name,from:{label:String(f.get('fromLabel')||'站位').trim(),x:readCoord('fromX'),y:readCoord('fromY')},target:{label:String(f.get('targetLabel')||'落点').trim(),x:readCoord('targetX'),y:readCoord('targetY')},technique:String(f.get('technique')||'').trim(),movement:String(f.get('movement')||'').trim(),airTime:String(f.get('airTime')||'').trim(),steps,result:String(f.get('result')||'').trim(),source,updated:today(),media:{poster,video},custom:true};
      if(!record.from.label||!record.target.label){if(status)status.textContent='请填写站位名称和落点名称。';return;}
      ...
      const persisted=save();tacticLineupEditingId=null;render({preserveScroll:true});toast(persisted?(existing?'投掷卡已更新。':'投掷卡已保存。'):'投掷卡已暂存，请导出备份。');return;
    }
