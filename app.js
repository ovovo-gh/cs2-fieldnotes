(() => {
  ...
  function tacticEffectMarkup(zone){
    const x=tacticPct(zone.x),y=tacticPct(zone.y),r=Math.max(1,Number(zone.r)||6),utility=zone.utility,side=zone.throwSide==='CT'?'ct':'t',timelineAttrs=zone.throwTime===undefined?'':` data-tactic-throw="${esc(zone.throwId||zone.id)}" data-throw-time="${esc(zone.throwTime)}" data-throw-side="${esc(zone.throwSide||'T')}"`;
    if(zone.throwSide)zone.label=`${zone.throwSide} · ${zone.label}`;
    if(utility==='fire'){
      const fireRx=Math.max(2,Number(zone.rx)||8),fireRy=Math.max(2,Number(zone.ry)||5);
      return `<g class="tactic-effect tactic-effect-fire tactic-effect-side-${side}" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"${timelineAttrs}><ellipse cx="${x}" cy="${y}" rx="${fireRx}" ry="${fireRy}"><title>${esc(zone.label)}</title></ellipse><ellipse class="fire-inner" cx="${x+1}" cy="${y-1}" rx="${Math.max(1,fireRx*.62)}" ry="${Math.max(1,fireRy*.58)}"></ellipse><ellipse class="flame-tongue flame-one" cx="${x-fireRx*.42}" cy="${y-fireRy*.2}" rx="${Math.max(.7,fireRx*.17)}" ry="${Math.max(1.2,fireRy*.7)}"></ellipse><ellipse class="flame-tongue flame-two" cx="${x+fireRx*.04}" cy="${y-fireRy*.4}" rx="${Math.max(.7,fireRx*.15)}" ry="${Math.max(1.2,fireRy*.85)}"></ellipse><ellipse class="flame-tongue flame-three" cx="${x+fireRx*.47}" cy="${y-fireRy*.12}" rx="${Math.max(.7,fireRx*.14)}" ry="${Math.max(1.2,fireRy*.58)}"></ellipse><text x="${x}" y="${Math.max(4,y-r*.55)}">${esc(zone.label)}</text></g>`;
    }
    if(utility==='flash'){
    ...
    if(utility==='he')return `<g class="tactic-effect tactic-effect-he tactic-effect-side-${side}" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"${timelineAttrs}><circle cx="${x}" cy="${y}" r="${r}"><title>${esc(zone.label)}</title></circle><circle class="blast-core" cx="${x}" cy="${y}" r="${Math.max(1.5,r*.22)}"></circle><circle class="blast-ring blast-ring-a" cx="${x}" cy="${y}" r="${Math.max(1,r*.3)}"></circle><circle class="blast-ring blast-ring-b" cx="${x}" cy="${y}" r="${Math.max(1,r*.5)}"></circle><path class="blast-burst" d="M ${x-r*.7} ${y} L ${x+r*.7} ${y} M ${x} ${y-r*.7} L ${x} ${y+r*.7}"></path><text x="${x}" y="${Math.max(4,y-r*.65)}">${esc(zone.label)}</text></g>`;
    return `<g class="tactic-effect tactic-effect-smoke tactic-effect-side-${side}" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"${timelineAttrs}><circle cx="${x}" cy="${y}" r="${r}"><title>${esc(zone.label)}</title></circle><circle class="smoke-core" cx="${x}" cy="${y}" r="${Math.max(1.5,r*.25)}"></circle><circle class="smoke-puff smoke-puff-a" cx="${x-r*.35}" cy="${y+r*.15}" r="${Math.max(1.2,r*.52)}"></circle><circle class="smoke-puff smoke-puff-b" cx="${x+r*.28}" cy="${y-r*.18}" r="${Math.max(1.2,r*.58)}"></circle><circle class="smoke-puff smoke-puff-c" cx="${x+r*.05}" cy="${y+r*.4}" r="${Math.max(1.2,r*.45)}"></circle><text x="${x}" y="${Math.max(4,y-r*.62)}">${esc(zone.label)}</text></g>`;
  }
  function tacticMapMarker(label,point,type){const x=tacticPct(point.x),y=tacticPct(point.y);return `<g class="map-marker map-marker-${type}" transform="translate(${x} ${y})"><text x="2.6" y="1.1">${esc(label)}</text></g>`;}
  ...
  function tacticThrowMarker(throwItem){const x=tacticPct(throwItem.x),y=tacticPct(throwItem.y),side=throwItem.side==='CT'?'ct':'t';return `<g class="tactic-throw-marker tactic-throw-marker-${side}" data-tactic-throw-marker="${esc(throwItem.id)}"><circle cx="${x}" cy="${y}" r="2.8"><title>${esc(throwItem.label)} · ${Number(throwItem.time).toFixed(1)} 秒投掷</title></circle><text x="${x+3.4}" y="${Math.max(3,y-3.2)}">${esc(Number(throwItem.time).toFixed(1))}s</text></g>`;}
