(() => {
  ...
  function tacticPlayerMarkup(player){const tint=player.side==='CT'?'#8ad7fa':'#ffbf7c',label=player.label||player.id;return `<g class="tactic-player-marker tactic-player-${player.side==='CT'?'ct':'t'}" data-tactic-player="${esc(player.id)}" transform="translate(${tacticPct(player.x)} ${tacticPct(player.y)})" tabindex="0" role="button" aria-label="${esc(player.side)} 方 ${esc(label)}，可拖动"><circle r="1.15" style="--player-color:${tint}"><title>${esc(player.side)} 方 ${esc(label)} · 可拖动调整走位</title></circle><text y=".52">${esc(label.replace(/^(?:CT|T)/,''))}</text></g>`;}
  ...
  function updateTacticTimelineUI(){
    ...
    $$('[data-tactic-effect]').forEach(effect=>{const {start,end}=tacticTimelineEffectWindow(effect),active=time>0&&time>=start&&time<end,future=time<Math.max(start,.01),expired=time>=end&&time>0;effect.classList.toggle('is-future',future);effect.classList.toggle('is-active',active);effect.classList.toggle('is-expired',expired);effect.style.display=expired?'none':'';effect.style.opacity=future?'.22':active?'1':expired?'.18':'.42';});
  ...
  function tacticEffectMarkup(zone){
    ...
    if(utility==='fire'){
      ...
      return `<g class="tactic-effect tactic-effect-fire" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"${timelineAttrs}><ellipse cx="${x}" cy="${y}" rx="${fireRx}" ry="${fireRy}"><title>${esc(zone.label)}</title></ellipse><ellipse class="fire-inner" cx="${x+1}" cy="${y-1}" rx="${Math.max(1,fireRx*.62)}" ry="${Math.max(1,fireRy*.58)}"></ellipse><ellipse class="flame-tongue flame-one" cx="${x-fireRx*.42}" cy="${y-fireRy*.2}" rx="${Math.max(.7,fireRx*.17)}" ry="${Math.max(1.2,fireRy*.7)}"></ellipse><ellipse class="flame-tongue flame-two" cx="${x+fireRx*.04}" cy="${y-fireRy*.4}" rx="${Math.max(.7,fireRx*.15)}" ry="${Math.max(1.2,fireRy*.85)}"></ellipse><ellipse class="flame-tongue flame-three" cx="${x+fireRx*.47}" cy="${y-fireRy*.12}" rx="${Math.max(.7,fireRx*.14)}" ry="${Math.max(1.2,fireRy*.58)}"></ellipse><text x="${x}" y="${Math.max(4,y-r*.55)}">${esc(zone.label)}</text></g>`;
    }
    if(utility==='flash'){
      ...
      return `<g class="tactic-effect tactic-effect-flash" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"${timelineAttrs}><path d="M ${x} ${y} L ${x1} ${y1} A ${r} ${r} 0 0 1 ${x2} ${y2} Z"><title>${esc(zone.label)}</title></path><circle cx="${x}" cy="${y}" r="1.7"></circle><circle class="flash-pulse" cx="${x}" cy="${y}" r="${Math.max(2,r*.18)}"></circle><text x="${x}" y="${Math.max(4,y-r*.58)}">${esc(zone.label)}</text></g>`;
    }
    if(utility==='he')return `<g class="tactic-effect tactic-effect-he" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"${timelineAttrs}><circle cx="${x}" cy="${y}" r="${r}"><title>${esc(zone.label)}</title></circle><circle class="blast-core" cx="${x}" cy="${y}" r="${Math.max(1.5,r*.22)}"></circle><circle class="blast-ring blast-ring-a" cx="${x}" cy="${y}" r="${Math.max(1,r*.3)}"></circle><circle class="blast-ring blast-ring-b" cx="${x}" cy="${y}" r="${Math.max(1,r*.5)}"></circle><path class="blast-burst" d="M ${x-r*.7} ${y} L ${x+r*.7} ${y} M ${x} ${y-r*.7} L ${x} ${y+r*.7}"></path><text x="${x}" y="${Math.max(4,y-r*.65)}">${esc(zone.label)}</text></g>`;
    return `<g class="tactic-effect tactic-effect-smoke" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"${timelineAttrs}><circle cx="${x}" cy="${y}" r="${r}"><title>${esc(zone.label)}</title></circle><circle class="smoke-core" cx="${x}" cy="${y}" r="${Math.max(1.5,r*.25)}"></circle><circle class="smoke-puff smoke-puff-a" cx="${x-r*.35}" cy="${y+r*.15}" r="${Math.max(1.2,r*.52)}"></circle><circle class="smoke-puff smoke-puff-b" cx="${x+r*.28}" cy="${y-r*.18}" r="${Math.max(1.2,r*.58)}"></circle><circle class="smoke-puff smoke-puff-c" cx="${x+r*.05}" cy="${y+r*.4}" r="${Math.max(1.2,r*.45)}"></circle><text x="${x}" y="${Math.max(4,y-r*.62)}">${esc(zone.label)}</text></g>`;
  }
  ...
  function tacticThrowZone(throwItem){const item=tacticUtilityById(throwItem.utility),defaults={smoke:{r:7},flash:{r:15,angle:0},he:{r:9},fire:{rx:8,ry:5}}[throwItem.utility]||{r:7};return {...defaults,...throwItem,id:`throw-${throwItem.id}`,label:throwItem.label||item?.name||throwItem.utility,throwId:throwItem.id,throwTime:throwItem.time,throwSide:throwItem.side};}
