(() => {
  ...
  let tacticsMapId='dust2',tacticPlaceUtility='smoke',tacticPlaceSide='T',tacticsLineupId='',tacticsPlacementMode=false,tacticThrowSerial=0;
  ...
  function tacticPlayerMarkup(player){const tint=player.side==='CT'?'#8ad7fa':'#ffbf7c',label=player.label||player.id;return `<g class="tactic-player-marker tactic-player-${player.side==='CT'?'ct':'t'}" data-tactic-player="${esc(player.id)}" transform="translate(${tacticPct(player.x)} ${tacticPct(player.y)})" tabindex="0" role="button" aria-label="${esc(player.side)} 方 ${esc(label)}，可拖动"><circle r="1.75" style="--player-color:${tint}"><title>${esc(player.side)} 方 ${esc(label)} · 可拖动调整走位</title></circle><text y=".72">${esc(label.replace(/^(?:CT|T)/,''))}</text></g>`;}
  ...
  function tacticTimelineDurationLabel(item){if(item?.id==='fire')return 'T 7.0 秒 / CT 5.5 秒';if(item?.id==='flash')return '1.5 秒后爆炸 · 最长 5 秒';if(item?.id==='he')return '1.5 秒后爆炸 · 瞬时';return item?.duration||'';}
  ...
  function updateTacticTimelineUI(){
    ...
    if(slider){slider.value=time;slider.setAttribute('aria-valuenow',time.toFixed(1));}
    if(readout)readout.textContent=`${time.toFixed(1)} / ${tacticTimelineMax.toFixed(1)} 秒`;
    ...
    $$('[data-tactic-effect]').forEach(effect=>{const {start,end}=tacticTimelineEffectWindow(effect),active=time>0&&time>=start&&time<end,future=time<Math.max(start,.01),expired=time>=end&&time>0;effect.classList.toggle('is-future',future);effect.classList.toggle('is-active',active);effect.classList.toggle('is-expired',expired);effect.style.opacity=future?'.22':active?'1':expired?'.18':'.42';});
    const mapTime=$('#tactic-map-live-time'),mapStatus=$('#tactic-map-live-status'),placeCue=$('.map-place-cue'),editorHint=$('.editor-hint');if(mapTime)mapTime.textContent=`时间轴 ${time.toFixed(1)} 秒`;if(mapStatus)mapStatus.textContent=tacticTimelineMapStatus(selectedTacticMap(),time);if(placeCue)placeCue.textContent=tacticsPlacementMode?`点击地图放置 · 当前时刻 ${time.toFixed(1)} 秒`:'先在下方设定时刻，再点击“点击地图添加”';if(editorHint)editorHint.textContent=`当前时间 ${time.toFixed(1)} 秒 · 选择“点击地图添加”后，在地图上点击实际落点；投掷时间和落点都会记录到下方时间轴。`;
  ...
  function tacticEffectMarkup(zone){
    const x=tacticPct(zone.x),y=tacticPct(zone.y),r=Math.max(1,Number(zone.r)||6),utility=zone.utility,timelineAttrs=zone.throwTime===undefined?'':` data-tactic-throw="${esc(zone.throwId||zone.id)}" data-throw-time="${esc(zone.throwTime)}" data-throw-side="${esc(zone.throwSide||'T')}"`;
    if(utility==='fire')return `<g class="tactic-effect tactic-effect-fire" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"${timelineAttrs}><ellipse cx="${x}" cy="${y}" rx="${Math.max(2,Number(zone.rx)||8)}" ry="${Math.max(2,Number(zone.ry)||5)}"><title>${esc(zone.label)}</title></ellipse><ellipse class="fire-inner" cx="${x+1}" cy="${y-1}" rx="${Math.max(1,Number(zone.rx||8)*.62)}" ry="${Math.max(1,Number(zone.ry||5)*.58)}"></ellipse><text x="${x}" y="${Math.max(4,y-r*.55)}">${esc(zone.label)}</text></g>`;
    if(utility==='flash'){
      ...
      return `<g class="tactic-effect tactic-effect-flash" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"${timelineAttrs}><path d="M ${x} ${y} L ${x1} ${y1} A ${r} ${r} 0 0 1 ${x2} ${y2} Z"><title>${esc(zone.label)}</title></path><circle cx="${x}" cy="${y}" r="1.7"></circle><text x="${x}" y="${Math.max(4,y-r*.58)}">${esc(zone.label)}</text></g>`;
    }
    if(utility==='he')return `<g class="tactic-effect tactic-effect-he" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"${timelineAttrs}><circle cx="${x}" cy="${y}" r="${r}"><title>${esc(zone.label)}</title></circle><circle class="blast-core" cx="${x}" cy="${y}" r="${Math.max(1.5,r*.22)}"></circle><text x="${x}" y="${Math.max(4,y-r*.65)}">${esc(zone.label)}</text></g>`;
    if(utility==='decoy')return `<g class="tactic-effect tactic-effect-decoy" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"${timelineAttrs}><circle cx="${x}" cy="${y}" r="${r}"><title>${esc(zone.label)}</title></circle><circle class="decoy-core" cx="${x}" cy="${y}" r="${Math.max(1.5,r*.2)}"></circle><text x="${x}" y="${Math.max(4,y-r*.65)}">${esc(zone.label)}</text></g>`;
    return `<g class="tactic-effect tactic-effect-smoke" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"${timelineAttrs}><circle cx="${x}" cy="${y}" r="${r}"><title>${esc(zone.label)}</title></circle><circle class="smoke-core" cx="${x}" cy="${y}" r="${Math.max(1.5,r*.25)}"></circle><text x="${x}" y="${Math.max(4,y-r*.62)}">${esc(zone.label)}</text></g>`;
  }
  ...
  function tacticThrowZone(throwItem){const item=tacticUtilityById(throwItem.utility),defaults={smoke:{r:7},flash:{r:15,angle:0},he:{r:9},fire:{rx:8,ry:5},decoy:{r:7}}[throwItem.utility]||{r:7};return {...defaults,...throwItem,id:`throw-${throwItem.id}`,label:throwItem.label||item?.name||throwItem.utility,throwId:throwItem.id,throwTime:throwItem.time,throwSide:throwItem.side};}
  function tacticThrowMarker(throwItem){const x=tacticPct(throwItem.x),y=tacticPct(throwItem.y);return `<g class="tactic-throw-marker" data-tactic-throw-marker="${esc(throwItem.id)}"><circle cx="${x}" cy="${y}" r="2.8"><title>${esc(throwItem.label)} · ${Number(throwItem.time).toFixed(1)} 秒投掷</title></circle><text x="${x+3.4}" y="${Math.max(3,y-3.2)}">${esc(Number(throwItem.time).toFixed(1))}s</text></g>`;}
  ...
  function lineupMedia(lineup){
    ...
    const source=media.video?`<source src="${esc(media.video)}" type="video/mp4">`:'';
    return `<figure class="lineup-media"><video controls preload="none" poster="${esc(media.poster)}" aria-label="${esc(lineup.name)} 实际投掷演示">${source}<img src="${esc(media.poster)}" alt="${esc(lineup.name)} 实际投掷截图" loading="lazy"></video><figcaption><strong>实际投掷画面</strong><span>可直接在卡片内播放完整过程</span></figcaption></figure>`;
  }
  function tacticLineupCard(lineup){
    const active=selectedTacticLineup()?.id===lineup.id;
    return `<article class="lineup-card ${active?'active':''}"><button class="lineup-select" data-lineup-id="${esc(lineup.id)}" aria-pressed="${active}"><div class="lineup-card-head"><span class="side-badge ${lineup.side==='CT'?'ct':''}">${esc(lineup.side)} 方</span><span class="lineup-method">${esc(lineup.technique)} · ${esc(lineup.airTime)}</span></div><h3>${esc(lineup.name)}</h3><p class="lineup-route-label">${esc(lineup.from.label)} <span>→</span> ${esc(lineup.target.label)}</p><span class="lineup-select-hint">${active?'地图上已显示这颗烟的路线':'点击在地图上显示这颗烟的路线'}</span></button>${lineupMedia(lineup)}<div class="lineup-facts"><span>移动：${esc(lineup.movement)}</span><span>精度：精准</span></div><ol>${lineup.steps.map(step=>`<li>${esc(step)}</li>`).join('')}</ol><p class="lineup-result"><strong>用途：</strong>${esc(lineup.result)}</p><a class="lineup-source" href="${esc(lineup.source)}" target="_blank" rel="noreferrer">查看原始投掷卡 · 更新 ${esc(lineup.updated)} ↗</a></article>`;
  }
  function tacticTimelineThrowRow(throwItem){
    const item=tacticUtilityById(throwItem.utility),start=tacticTimelineThrowStart(throwItem),duration=tacticTimelineThrowEnd(throwItem)-start,startPct=Math.min(100,Math.max(0,start/tacticTimelineMax*100)),width=Math.max(.8,Math.min(100-startPct,duration/tacticTimelineMax*100)),timing=item?.id==='fire'?(throwItem.side==='CT'?'CT 5.5 秒':'T 7.0 秒'):(item?.duration||'');
    return `<div class="tactic-timeline-row" data-tactic-track="${esc(throwItem.id)}" data-utility="${esc(throwItem.utility)}" data-throw-time="${esc(throwItem.time)}" data-throw-side="${esc(throwItem.side)}"><div class="tactic-track-label"><span class="utility-icon" style="--utility-color:${esc(item?.color||'#c8dd91')}">${esc(item?.icon||'◎')}</span><span><strong>${esc(throwItem.label||item?.name||throwItem.utility)}</strong><small>${esc(throwItem.side)} 方 · 投掷 ${Number(throwItem.time).toFixed(1)} 秒 · ${esc(timing)}</small></span></div><div class="tactic-track-bar" aria-hidden="true"><span class="tactic-track-fill" style="left:${startPct}%;width:${width}%"></span><span class="tactic-throw-time-marker" style="left:${Math.min(100,Math.max(0,Number(throwItem.time)/tacticTimelineMax*100))}%"></span><span class="tactic-track-cursor" style="left:${tacticsTimeline.time/tacticTimelineMax*100}%"></span></div><span class="tactic-track-state">${esc(tacticTimelineThrowState(throwItem,tacticsTimeline.time))}</span><button class="tactic-delete-throw" data-delete-throw="${esc(throwItem.id)}" type="button" aria-label="删除 ${esc(throwItem.label||item?.name||throwItem.utility)}">×</button></div>`;
  }
  ...
  function tactics(){
    ...
    return heading('MAP TACTICS / MAP BOARD','地图战术板','沙2、Mirage、小镇：从一张空地图开始，把道具、落点、投掷时间和组合都编排成自己的战术方案。',`<span class="date-badge">数据快照 · ${esc(tacticsData.snapshot)}</span>`)+
      ...
      `<section class="panel tactics-lineups" id="tactics-lineups"><div class="section-head"><div><h2>${esc(map.name)} · 关键投掷办法</h2><p class="tiny">点击卡片后，路线会作为单独的教学参考显示在地图上；每张卡直接展示实际投掷截图 / 视频，不会自动写入你的方案时间轴。</p></div><span class="tiny">${lineups.length} 张卡</span></div><div class="lineup-grid">${lineups.map(tacticLineupCard).join('')}</div><div class="tactics-practice-note"><strong>练习顺序：</strong>先在练习房打开无限弹药 / 无限手雷，逐张卡确认站位和落点；地图碰撞、出生位和跳投行为可能随 Valve 更新改变，卡片上的“更新日期”用于提醒你复核。画面来自 CSNADES.gg，来源链接仍保留在每张卡底部。</div></section>`+
      `<section class="panel tactics-sources"><div class="section-head"><div><h2>数据口径与来源</h2><p class="tiny">把可验证的地图 / 道具数据与教学投掷卡分开标注。</p></div><span class="tiny">版本快照 ${esc(tacticsData.snapshot)}</span></div><div class="source-grid"><div><strong>地图底图与坐标</strong><p>三张 PNG 是从官方游戏 depot 同步的雷达快照；overview 的 pos_x、pos_y、scale 和包点 / 出生点归一化坐标随地图一并固定。</p><a href="${esc(tacticsData.radarSource)}" target="_blank" rel="noreferrer">CS2 Map Icons / depot 同步说明 ↗</a><br><a href="${esc(map.overviewSource)}" target="_blank" rel="noreferrer">当前地图 overview：${esc(map.overviewFile)} ↗</a></div><div><strong>道具规则</strong><p>道具分类、游戏实体名与投掷速度按 Valve 的 GameTracking-CS2 数据核对；持续时间按当前竞技模式常用游戏规则记录。真实可见性、伤害、火焰扩散和碰撞仍受距离、视角、材质与地图几何影响。</p><a href="${esc(tacticsData.utilitySource)}" target="_blank" rel="noreferrer">Valve GameTracking / weapons.vdata ↗</a></div><div><strong>投掷卡</strong><p>关键投掷卡保留可追溯的 CSNADES 单颗投掷页，卡片内直接显示原始投掷画面；你可以先看图确认手法，再把最终落点和时刻手动编排进地图方案。</p><a href="${esc(tacticsData.lineupSource)}" target="_blank" rel="noreferrer">CSNADES 当前地图指南 ↗</a><br><span class="tiny">地图底图版权归 Valve Corporation；本项目不是 Valve 官方产品。</span></div></div></section>`;
  }
  function dashboard(){
  ...
  function drawRecoil(now=performance.now()){
    ...
    const persistedSamples=session?.sampleTrace?.length?session.sampleTrace:session?.trace||[],toBullet=p=>({x:origin.x+p.x*scale,y:origin.y+p.y*scale}),toIdeal=p=>({x:origin.x-p.x*scale,y:origin.y-p.y*scale}),bullet=reference.map(toBullet),ideal=reference.map(toIdeal),active=!!session?.active,samples=active?(session.samples||[]):persistedSamples.slice(0,reference.length).map((actual,index)=>({shot:index+1,actual,error:session?.shotErrors?.[index]||0,late:(session?.shotErrors?.[index]||0)>recoilErrorThreshold})),firedCount=samples.length;
  ...
  function render({preserveScroll=false}={}){
    ...
    if(route==='tactics')updateTacticTimelineUI();
  ...
  function startTacticPlayerDrag(event,marker){const board=marker.closest?.('#tactic-map-canvas'),playerId=marker.dataset.tacticPlayer;if(!board||!playerId||!tacticPlayerById(tacticsMapId,playerId))return;tacticDraggingPlayer={id:playerId,pointerId:event.pointerId,board,marker};marker.setAttribute('aria-grabbed','true');try{board.setPointerCapture(event.pointerId);}catch{}event.preventDefault();}
  ...
  function addTacticThrow(event,board){
    ...
    const point=tacticMapPointFromEvent(event,board),label=String($('#tactic-place-label')?.value||'').trim()||`${item.name} · ${tacticsTimeline.time.toFixed(1)} 秒`,throwItem={id:`${map.id}-throw-${Date.now()}-${++tacticThrowSerial}`,utility:item.id,side,x:Number(point.x.toFixed(1)),y:Number(point.y.toFixed(1)),time:Number(tacticsTimeline.time.toFixed(1)),label};
    tacticPlanFor(map.id).push(throwItem);tacticsPlacementMode=false;render({preserveScroll:true});toast(`已添加 ${item.name}：${throwItem.time.toFixed(1)} 秒投掷，地图坐标 ${throwItem.x.toFixed(1)} / ${throwItem.y.toFixed(1)}。`);
  }
  ...
  document.addEventListener('click',e=>{
    const board=e.target.closest?.('#tactic-map-canvas'),player=e.target.closest?.('[data-tactic-player]');if(board&&tacticsPlacementMode&&!player){addTacticThrow(e,board);return;}
    ...
    if(t.id==='tactic-place-toggle'){tacticsPlacementMode=!tacticsPlacementMode;render({preserveScroll:true});return;}
    ...
    if(t.id==='tactic-plan-clear'){tacticPlanFor(tacticsMapId).splice(0);tacticsLineupId='';tacticsPlacementMode=false;render({preserveScroll:true});toast('当前地图方案已清空。');return;}
    if(t.dataset.deleteThrow){removeTacticThrow(t.dataset.deleteThrow);render({preserveScroll:true});return;}
    if(t.dataset.mapId){if(tacticMaps.some(map=>map.id===t.dataset.mapId)){tacticsMapId=t.dataset.mapId;tacticsLineupId='';tacticsPlacementMode=false;render();}return;}
