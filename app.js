(() => {
  ...
  function updateTacticTimelineUI(){
    ...
    const mapTime=$('#tactic-map-live-time'),mapStatus=$('#tactic-map-live-status'),placeCue=$('.map-place-cue'),editorHint=$('.editor-hint');if(mapTime)mapTime.textContent=`时间轴 ${time.toFixed(1)} 秒`;if(mapStatus)mapStatus.textContent=tacticTimelineMapStatus(selectedTacticMap(),time);if(placeCue)placeCue.textContent=tacticsPlacementMode?`点击地图放置 · 当前时刻 ${time.toFixed(1)} 秒`:'';if(editorHint)editorHint.textContent=`当前时间 ${time.toFixed(1)} 秒 · 选择“点击地图添加”后，在地图上点击实际落点；投掷时间和落点都会记录到下方时间轴。`;
  ...
  function tacticMapMarker(label,point,type){const x=tacticPct(point.x),y=tacticPct(point.y);return `<g class="map-marker map-marker-${type}" transform="translate(${x} ${y})"><text x="-2.6" y="1.1" text-anchor="end">${esc(label)}</text></g>`;}
