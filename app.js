(() => {
  ...
  function tacticTimelineDuration(itemOrId,side=tacticsTimeline.side){const item=typeof itemOrId==='string'?tacticUtilityById(itemOrId):itemOrId,duration=item?.durationSeconds;if(duration&&typeof duration==='object')return Number(duration[side]||duration.T||7);return Number(duration)||0;}
  function tacticTimelineDurationLabel(item){if(item?.id==='fire')return 'T 7.0 秒 / CT 5.5 秒';return item?.duration||'';}
  function tacticTimelineState(item,time){if(time<=0)return '待投掷';if(item.id==='he')return time<tacticTimelineDuration(item)?'引信中':'已爆炸';if(item.id==='flash')return time<tacticTimelineDuration(item)?'致盲窗口':'已结束';return time<tacticTimelineDuration(item)?'生效中':'已结束';}
  function tacticTimelineStrategy(time=tacticsTimeline.time,side=tacticsTimeline.side){const fire=tacticTimelineDuration('fire',side);if(time<=0)return '<strong>准备阶段：</strong>按下播放后，从 0 秒开始观察“投掷 → 生效 → 结束”。先把烟、火的剩余时间和闪光 / HE 的短窗口对齐，再决定是否进点。';if(time<1.5)return '<strong>0–1.5 秒：</strong>HE / 闪光仍在引信窗口；不要把尚未爆炸的道具当作已经创造了空间，跟进者应先等确认或利用掩体。';if(time<5)return '<strong>1.5–5 秒：</strong>闪光的最长致盲窗口仍可能存在，HE 已完成爆炸；这是短促跟进、清近点和穿过已生效烟火边缘的时间。';if(time<fire)return `<strong>5–${fire.toFixed(1)} 秒：</strong>${side==='T'?'T 方火焰':'CT 方火焰'}仍在封锁区域；闪光窗口已结束，烟雾和火焰的组合更适合拖延、隔离枪线或分割进点。`;if(time<15)return `<strong>${fire.toFixed(1)}–15 秒：</strong>火焰已经结束，但烟雾仍可持续遮挡；${side==='T'?'进攻方要抓住烟边转移或补第二颗烟。':'防守方要利用烟散前的时间回到交叉火力。'}诱饵声音仍可能制造错误信息。`;if(time<18)return '<strong>15–18 秒：</strong>诱饵声音周期结束，场上主要只剩烟雾；开始为下一轮枪线、补烟或撤退路线做准备。';return '<strong>18 秒后：</strong>标准烟雾周期结束。把这个时间点当作重新争夺视线、转点或结束拖延的提醒。';}
  ...
  function updateTacticTimelineUI(){
    ...
    $$('[data-tactic-track]').forEach(row=>{const item=tacticUtilityById(row.dataset.tacticTrack),duration=tacticTimelineDuration(item),cursor=$('.tactic-track-cursor',row),fill=$('.tactic-track-fill',row),stateNode=$('.tactic-track-state',row);if(cursor)cursor.style.left=`${ratio*100}%`;if(fill)fill.style.width=`${Math.min(100,duration/tacticTimelineMax*100)}%`;row.classList.toggle('is-active',time>0&&time<duration);row.classList.toggle('is-expired',time>=duration&&time>0);if(stateNode)stateNode.textContent=tacticTimelineState(item,time);});
    $$('[data-tactic-effect]').forEach(effect=>{const duration=tacticTimelineDuration(effect.dataset.utility),active=time>0&&time<duration;effect.classList.toggle('is-future',time<=0);effect.classList.toggle('is-active',active);effect.classList.toggle('is-expired',time>=duration&&time>0);effect.style.opacity=time<=0?'.78':active?'1':'.18';});
  ...
  function drawRecoil(now=performance.now()){
    ...
    if(active&&session.startedAt!==null){
      ...
      if(session.currentShot<session.shots){
        ...
        ctx.fillStyle='#ffe08a';ctx.fillText(`第 ${index+1} 发目标 · ${ready?'等待开始':`${(session.intervalMs*(1-progress)).toFixed(0)} ms`}`,moving.x+18,moving.y-10);
      }
    }
