(() => {
  ...
  function tacticTimelineMapStatus(map=selectedTacticMap(),time=tacticsTimeline.time){const zones=(map?.effects||[]).filter(zone=>tacticsUtility==='all'||zone.utility===tacticsUtility),counts=zones.reduce((result,zone)=>{const item=tacticUtilityById(zone.utility),start=tacticTimelineStart(item),end=tacticTimelineEnd(item);if(time>0&&time>=start&&time<end)result.active+=1;else if(time<Math.max(start,.01))result.future+=1;else result.expired+=1;return result;},{active:0,future:0,expired:0});if(time<=0)return '准备阶段 · 点击播放或拖动时间轴';return `当前生效 ${counts.active} 个 · 待触发 ${counts.future} 个 · 已结束 ${counts.expired} 个`;}
  ...
  function updateTacticTimelineUI(){
    ...
    $$('[data-tactic-track]').forEach(row=>{const item=tacticUtilityById(row.dataset.tacticTrack),start=tacticTimelineStart(item),duration=tacticTimelineDuration(item),end=start+duration,cursor=$('.tactic-track-cursor',row),fill=$('.tactic-track-fill',row),stateNode=$('.tactic-track-state',row);if(cursor)cursor.style.left=`${ratio*100}%`;if(fill){fill.style.left=`${start/tacticTimelineMax*100}%`;fill.style.width=`${Math.min(100-start/tacticTimelineMax*100,Math.max(.8,duration/tacticTimelineMax*100))}%`;}row.classList.toggle('is-active',time>0&&time>=start&&time<end);row.classList.toggle('is-expired',time>=end&&time>0);if(stateNode)stateNode.textContent=tacticTimelineState(item,time);});
    $$('[data-tactic-effect]').forEach(effect=>{const item=tacticUtilityById(effect.dataset.utility),start=tacticTimelineStart(item),end=tacticTimelineEnd(item),active=time>0&&time>=start&&time<end,future=time<Math.max(start,.01),expired=time>=end&&time>0;effect.classList.toggle('is-future',future);effect.classList.toggle('is-active',active);effect.classList.toggle('is-expired',expired);effect.style.opacity=time<=0?'.78':future?'.42':active?'1':'.18';});
    const mapTime=$('#tactic-map-live-time'),mapStatus=$('#tactic-map-live-status');if(mapTime)mapTime.textContent=`时间轴 ${time.toFixed(1)} 秒`;if(mapStatus)mapStatus.textContent=tacticTimelineMapStatus(selectedTacticMap(),time);
  ...
  function tacticEffectMarkup(zone){
    ...
    if(utility==='fire')return `<g class="tactic-effect tactic-effect-fire" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"><ellipse cx="${x}" cy="${y}" rx="${Math.max(2,Number(zone.rx)||8)}" ry="${Math.max(2,Number(zone.ry)||5)}"><title>${esc(zone.label)}</title></ellipse><ellipse class="fire-inner" cx="${x+1}" cy="${y-1}" rx="${Math.max(1,Number(zone.rx||8)*.62)}" ry="${Math.max(1,Number(zone.ry||5)*.58)}"></ellipse><text x="${x}" y="${Math.max(4,y-r*.55)}">${esc(zone.label)}</text></g>`;
    if(utility==='flash'){
      ...
      return `<g class="tactic-effect tactic-effect-flash" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"><path d="M ${x} ${y} L ${x1} ${y1} A ${r} ${r} 0 0 1 ${x2} ${y2} Z"><title>${esc(zone.label)}</title></path><circle cx="${x}" cy="${y}" r="1.7"></circle><text x="${x}" y="${Math.max(4,y-r*.58)}">${esc(zone.label)}</text></g>`;
    }
    if(utility==='he')return `<g class="tactic-effect tactic-effect-he" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"><circle cx="${x}" cy="${y}" r="${r}"><title>${esc(zone.label)}</title></circle><circle class="blast-core" cx="${x}" cy="${y}" r="${Math.max(1.5,r*.22)}"></circle><text x="${x}" y="${Math.max(4,y-r*.65)}">${esc(zone.label)}</text></g>`;
    if(utility==='decoy')return `<g class="tactic-effect tactic-effect-decoy" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"><circle cx="${x}" cy="${y}" r="${r}"><title>${esc(zone.label)}</title></circle><circle class="decoy-core" cx="${x}" cy="${y}" r="${Math.max(1.5,r*.2)}"></circle><text x="${x}" y="${Math.max(4,y-r*.65)}">${esc(zone.label)}</text></g>`;
    return `<g class="tactic-effect tactic-effect-smoke" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"><circle cx="${x}" cy="${y}" r="${r}"><title>${esc(zone.label)}</title></circle><circle class="smoke-core" cx="${x}" cy="${y}" r="${Math.max(1.5,r*.25)}"></circle><text x="${x}" y="${Math.max(4,y-r*.62)}">${esc(zone.label)}</text></g>`;
  }
  ...
  function tacticTimelineRow(item){
    const start=tacticTimelineStart(item),duration=tacticTimelineDuration(item),width=Math.min(100-start/tacticTimelineMax*100,Math.max(.8,duration/tacticTimelineMax*100));
    return `<div class="tactic-timeline-row" data-tactic-track="${esc(item.id)}"><div class="tactic-track-label"><span class="utility-icon" style="--utility-color:${esc(item.color)}">${esc(item.icon)}</span><span><strong>${esc(item.name)}</strong><small>${esc(tacticTimelineDurationLabel(item))}</small></span></div><div class="tactic-track-bar" aria-hidden="true"><span class="tactic-track-fill" style="left:${start/tacticTimelineMax*100}%;width:${width}%"></span><span class="tactic-track-cursor" style="left:${tacticsTimeline.time/tacticTimelineMax*100}%"></span></div><span class="tactic-track-state">${esc(tacticTimelineState(item,tacticsTimeline.time))}</span></div>`;
  }
  function tacticsTimelineMarkup(){
    return `<section class="panel tactics-timeline" id="tactic-timeline"><div class="section-head"><div><h2>回合时间轴</h2><p class="tiny">拖动或播放 0–18 秒；橙色游标、地图上的彩色效果和状态栏会同步变化。</p></div><span class="timeline-limit">标准烟雾周期 · 18 秒</span></div><div class="tactic-timeline-toolbar"><button class="primary" id="tactic-timeline-play" type="button" aria-pressed="${tacticsTimeline.running}">${tacticsTimeline.running?'暂停时间轴':'播放时间轴'}</button><button class="secondary" id="tactic-timeline-reset" type="button">重置到 0 秒</button><label class="timeline-side">火焰口径<select id="tactic-side" aria-label="选择火焰阵营口径"><option value="T" ${tacticsTimeline.side==='T'?'selected':''}>T · Molotov 7.0 秒</option><option value="CT" ${tacticsTimeline.side==='CT'?'selected':''}>CT · Incendiary 5.5 秒</option></select></label><span class="timeline-readout" id="tactic-time-readout">${tacticsTimeline.time.toFixed(1)} / ${tacticTimelineMax.toFixed(1)} 秒</span></div><div class="tactic-slider-wrap"><input id="tactic-timeline-slider" type="range" min="0" max="${tacticTimelineMax}" step="0.1" value="${tacticsTimeline.time}" aria-label="回合时间轴，单位秒" aria-valuemin="0" aria-valuemax="${tacticTimelineMax}" aria-valuenow="${tacticsTimeline.time.toFixed(1)}"><div class="tactic-time-scale"><span>0</span><span>5</span><span>10</span><span>15</span><span>18 秒</span></div></div><div class="tactic-timeline-tracks">${tacticUtilityTypes.map(tacticTimelineRow).join('')}</div><div class="tactics-strategy-note" id="tactic-strategy-note">${tacticTimelineStrategy()}</div><p class="tiny timeline-footnote">时间轴展示的是道具效果的最长 / 标准窗口：闪光的实际致盲取决于距离、视角和遮挡；HE 的爆炸是瞬时事件；火焰范围和烟雾可见性还会受地面、碰撞与后续道具互动影响。</p></section>`;
  }
  function tactics(){
  ...
  function render({preserveScroll=false}={}){
    ...
    if(route!=='tactics'&&tacticsTimeline.running)stopTacticsTimeline();
