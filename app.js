(() => {
  ...
  function tacticEffectMarkup(zone){
    ...
    if(utility==='flash'){
      ...
      return `<g class="tactic-effect tactic-effect-flash tactic-effect-side-${side}" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"${timelineAttrs}><path d="M ${x} ${y} L ${x1} ${y1} A ${r} ${r} 0 0 1 ${x2} ${y2} Z"><title>${esc(zone.label)}</title></path><circle class="flash-pulse" cx="${x}" cy="${y}" r="${Math.max(2,r*.18)}"></circle><text x="${x}" y="${Math.max(4,y-r*.58)}">${esc(zone.label)}</text></g>`;
    }
    if(utility==='he')return `<g class="tactic-effect tactic-effect-he tactic-effect-side-${side}" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"${timelineAttrs}><title>${esc(zone.label)}</title><circle class="blast-core" cx="${x}" cy="${y}" r="${Math.max(1.4,r*.16)}"></circle><circle class="blast-ring blast-ring-a" cx="${x}" cy="${y}" r="${Math.max(1,r*.22)}"></circle><circle class="blast-ring blast-ring-b" cx="${x}" cy="${y}" r="${Math.max(1,r*.36)}"></circle><path class="blast-burst" d="M ${x-r*.52} ${y} L ${x+r*.52} ${y} M ${x} ${y-r*.52} L ${x} ${y+r*.52}"></path><text x="${x}" y="${Math.max(4,y-r*.65)}">${esc(zone.label)}</text></g>`;
    return `<g class="tactic-effect tactic-effect-smoke tactic-effect-side-${side}" data-tactic-effect="${esc(zone.id)}" data-utility="${esc(utility)}"${timelineAttrs}><title>${esc(zone.label)}</title><circle class="smoke-core" cx="${x}" cy="${y}" r="${Math.max(1.5,r*.25)}"></circle><circle class="smoke-puff smoke-puff-a" cx="${x-r*.35}" cy="${y+r*.15}" r="${Math.max(1.2,r*.52)}"></circle><circle class="smoke-puff smoke-puff-b" cx="${x+r*.28}" cy="${y-r*.18}" r="${Math.max(1.2,r*.58)}"></circle><circle class="smoke-puff smoke-puff-c" cx="${x+r*.05}" cy="${y+r*.4}" r="${Math.max(1.2,r*.45)}"></circle><text x="${x}" y="${Math.max(4,y-r*.62)}">${esc(zone.label)}</text></g>`;
  }
