...
在“地图战术板”查看沙2、Mirage、小镇的官方雷达平面图，筛选烟雾、闪光、雷和火焰的效果示意，并查看单人上限、五人队伍理论上限与关键投掷卡。地图 PNG 和 overview 坐标按 2026-09-11 快照固定在 `maps/` 与 `tactics-data.js`；投掷卡链接到可追溯的单颗投掷页，卡面路线图是教学示意，不替代当前练习房复核。

“关键投掷办法”区分内置资料和“我的投掷卡”：支持按关键词 / 道具搜索、新增、编辑、删除。自定义卡新增时会打开地图点选弹窗，只需先后点击投掷点和落点，坐标自动记录，不需要手填数字；还可以填写投掷步骤、来源、点位截图和演示视频地址，截图会直接显示在卡片内，视频也能直接播放。页面按地图整理了小黑盒公开检索入口，用于发现点位和截图；社区帖子不是版本锁定数据，加入自己的固定方案前请在当前 CS2 练习房复核。

记录（包括“我的投掷卡”）仅保存在当前浏览器，不会上传到 GitHub，也不会跨设备自动同步。请通过“备份与数据管理”定期导出 JSON；换设备时可导入，冲突记录以备份为准。
...
- `tactics-data.js`：沙2、Mirage、小镇的官方雷达快照元数据、道具规则、效果示意、内置关键投掷卡与小黑盒检索入口。
...
地图战术板的数量口径为：烟雾、HE、燃烧瓶 / 燃烧弹每位玩家最多 1 颗，五人队伍理论上限 5 颗；闪光每位玩家最多 2 颗，五人队伍理论上限 10 颗。地图上的范围按道具参考游戏单位结合各张 overview 的 `scale` 换算到 1024×1024 雷达图：烟雾约 144u 半径、HE 为 Valve 数据中的 350u 爆炸范围、火焰最大扩散距离为 150u；闪光用约 300u 的方向扇形提示，实际致盲仍由距离、视角、遮挡和地图几何决定。
